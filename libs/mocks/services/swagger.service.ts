...
export class SwaggerService {
    ...
    static buildDtoObject(method: RequestMethodEnum, resource: string) {
        ...
        const standardizedResource = capitalizedResource.slice(0, capitalizedResource.length - 1);
    ...
    static buildPathObject(endpoint: string, method: RequestMethodEnum, body?: CreatePropertyDto[]): PathsObject {
            ...
            operationObject.requestBody = body
                ? {
                      ...
                      content: {
                          'application/json': {
                              schema: {
                                  $ref: `#/components/schemas/${SwaggerService.buildDtoObject(method, resource)}`
                              }
                          }
                      }
                  }
    ...
    private mapSchemaFakerType(schema: SchemaObject, fakerType: FakerType): SchemaObject {
    ...
    private mapSchemaEnumType(schema: SchemaObject, items: CreatePropertyDto[]): SchemaObject {
        schema.type = TypeSelectionEnum.STRING.toLowerCase();
    ...
    private mapSchemaObjectType(schema: SchemaObject, items: CreatePropertyDto[]): SchemaObject {
    ...
    private mapSchemaArrayOfObjectType(schema: SchemaObject, items: CreatePropertyDto[]): SchemaObject {
        schema.type = 'array';
        for (const nestedProperty of items) {
            if (!schema.items) {
                schema.items = {};
    ...
    private mapSchemaArrayOfPrimitiveType(schema: SchemaObject, items: CreatePropertyDto[]): SchemaObject {
        schema.type = 'array';
        for (const nestedProperty of items) {
            if (!schema.items) {
                schema.items = {};

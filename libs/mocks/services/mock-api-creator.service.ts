...
export class MockApiCreatorService {
    constructor(
        @Inject(MOCK_API_PATH) private mockApiPath,
        @Inject(MAXIMUM_RANDOM_RESPONSE) private randomDataCount,
    ...
    static generateNestedProperty(nestedProperty: CreatePropertyDto) {
        ...
        return key
            ? { [key]: MockApiCreatorService.generateValueByType(type, fakerType, overrideValue) }
            : MockApiCreatorService.generateValueByType(type, fakerType, overrideValue);
    }
    ...
    private generateData(properties: CreatePropertyDto[]) {
        ...
        for (const property of properties) {
            ...
            if (isPrimitive(type)) {
                responses[key] = MockApiCreatorService.generateValueByType(type, fakerType, overrideValue);
            }
    ...
    private generateResponse(properties: CreatePropertyDto[], times: number = this.randomDataCount) {
        const responses = [];
    ...
    private parseMockApiJsonFile() {
        return JSON.parse(readFileSync(this.mockApiPath, { encoding: 'utf-8' }));
    }

    private saveMockApiJSON(mockApiJSON) {
        writeFileSync(this.mockApiPath, JSON.stringify(mockApiJSON));
    }
